...
def notes(request):
    ...
    par = (
        '<script type="text/javascript">$("#jstree").jstree(' + res_json + ");</script>"
    )
...
def my_notes(request):
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "select CAST(id as text) as idn,iif(parent_id==-1,'#',CAST(parent_id as text)) as parent,name as 'text' from note_note where user_id=="
                + str(request.user.id)
                + " order by level,name "
            )
...
def new_note(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return JsonResponse(
                {"id": str(nwNote.id), "parent": "#", "name": nwNote.name}, status=200
            )
